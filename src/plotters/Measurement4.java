...
public class Measurement4 {
	...
	static final int REQUESTS_PER_SAMPLE 		= 250;
	...
	public static void main(String[] args) throws Exception {
		...
		final JFreeChart chartCpusUsages 	= ChartFactory.createXYLineChart(PLOT1_TITLE, PLOT1_X_AXIS_LABEL, PLOT1_Y_AXIS_LABEL, datasets[0]);
		threadPool.shutdown();
	...
	private static XYDataset[] createDataset() throws Exception {		
		XYSeries[] avgResponseTime = new XYSeries[MAX_THREADS];
		XYSeries[] avgReponseModel = new XYSeries[MAX_THREADS];
		XYSeries[] cpuUsage		   = new XYSeries[MAX_THREADS];
		...
		for (int i=0; i<MAX_THREADS; i++) {
			...
			simulate(avgResponseTime[i], avgReponseModel[i], cpuUsage[i]);
		}
	...
	private static void simulate(XYSeries responseTime, XYSeries responseTimeModel, XYSeries cpusUsage) throws Exception {		
		for (int requestRate=MIN_REQUEST_RATE; requestRate <= MAX_REQUEST_RATE; requestRate+=REQUEST_RATE_INCREMENT) {
			...
			responseTimeModel.add(requestRate, model(requestRate));
		}
	}
